...
const UpdateCustomer = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Filtering properties with '' values
    const filtered = Object.entries(formData)
        .filter(([, value]) => value !== '')
        .reduce((acc, [key, value]) => ({...acc, [key]: value}), {});
    filtered.address = Object.entries(formData.address)
        .filter(([, value]) => value !== '')
        .reduce((acc, [key, value]) => ({...acc, [key]: value}), {});
    if(Object.keys(filtered.address).length == 0){
        delete filtered.address;
    }
    
    axios.put('/customers/update', filtered, {
